...
export class HzSystemWebSocketAPI {
  ...
  constructor(
    baseUrl: string = 'ws://192.168.124.3:8002/ws/detection/',
    ...
    try {
      ...
      switch (message.type) {
        case 'detection_start':
          this.eventHandlers.onDetectionStart?.(message.data as DetectionStartData)
          break
          
        case 'detection_progress':
          this.eventHandlers.onDetectionProgress?.(message.data as DetectionProgressData)
          break
          
        case 'detection_complete':
          this.eventHandlers.onDetectionComplete?.(message.data as DetectionCompleteData)
          break
          
        case 'detection_error':
          this.eventHandlers.onDetectionError?.(message.data as DetectionErrorData)
          break
          
        case 'system_notification':
          this.eventHandlers.onSystemNotification?.(message.data as SystemNotificationData)
          break
          
        case 'user_message_response':
          this.eventHandlers.onUserMessageResponse?.(message.data as UserMessageResponseData)
          break
          
        case 'connection_established':
          this.eventHandlers.onConnectionEstablished?.(message.data as ConnectionEstablishedData)
          break
          
        case 'connection_closing':
          this.eventHandlers.onConnectionClosing?.(message.data as ConnectionClosingData)
          break
